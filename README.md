...
### 第三阶段：工程化 / 健壮性 / 移动端
- **纯规则内核 `docs/js/rules.js`**：零 DOM、零全局的 ES Module（走子规则、将军/白脸将、合法着法、中文记谱、FEN/UCI），浏览器与 Node 共用同一份代码
- **单元测试**：`tests/` 用 Node 内置 `node:test`（零第三方依赖），覆盖蹩马腿/塞象眼/炮架/九宫/送将/记谱/FEN 等 14 组用例，`npm test` 运行
- **持续集成**：GitHub Actions（`.github/workflows/ci.yml`）在 Node 18/20/22 上自动跑内联模块语法检查、JS 语法检查与全部单测
- **常用命令**：`npm run verify`（语法门+单测一把过）、`npm run serve`（零依赖本地预览，正确返回 wasm MIME）、`npm test`
- **健壮性**：localStorage 存档损坏自动重置不白屏；对局列表文本转义防注入；引擎 Worker 运行期崩溃自动重启、连续崩溃稳定降级本地 AI；Service Worker 检测到新版本浮出“点击刷新”提示
- **移动端**：棋子支持手指拖拽落子（Pointer Events，跟手移动、非法落点回弹），同时保留点选；桌面鼠标同样可拖
